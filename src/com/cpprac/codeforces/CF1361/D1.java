...
public class D1 {
    ...
    public void solve() {
        ...
        ArrayList<Integer> topics[] = new ArrayList[n + 2];
        for (int i = 0; i < n; i++) {
        ...
        int visitedAt[] = new int[n + 2];
        ArrayList<Integer> ans = new ArrayList<>();
        int curr = 0;

        for (int i = 1; i <= maxTopic; i++) {

            ArrayList<Integer> nodesWithTopic = topics[i];

            for (Integer aNodesWithTopic : nodesWithTopic) {
                curr++;
                ans.add(aNodesWithTopic);
                ArrayList<Integer> neighboursOfj = graph[aNodesWithTopic];
                int count = 1;
                if ( neighboursOfj != null ) {
                    for (Integer iter : neighboursOfj) {
                        if ( ti[iter] == i ) {
                            notPossible(out);
                        }
                        if ( ti[iter] < i && visitedAt[ti[iter]] != curr ) {
                            visitedAt[ti[iter]] = curr;
                            count++;
                        }
                    }
                }
    ...
    private void notPossible(OutputWriter out) {
        out.printLine(-1);
        out.flush();
        System.exit(0);
    }
